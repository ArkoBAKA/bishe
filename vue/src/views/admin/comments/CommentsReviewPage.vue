    ...
    <div class="panel">
      ...
      <div v-else class="rows">
        <div v-for="r in list" :key="r.reportId" class="row">
          <div class="row-main">
            <div class="row-title">
              #{{ r.reportId }} 评论举报 commentId: {{ r.targetId }}
              <span class="pill" :class="{ done: r.status === 'processed' }">{{ r.status === 'processed' ? "已处理" : "待处理" }}</span>
            </div>
            <div class="row-sub">
          ...
          <div class="row-actions">
            <template v-if="r.status === 'pending'">
              <button class="btn danger" type="button" :disabled="actingId === r.reportId" @click="process(r.reportId, 'deleteComment')">
                删评结案
              </button>
              <button class="btn ok" type="button" :disabled="actingId === r.reportId" @click="process(r.reportId, 'close')">
                仅结案
              </button>
            </template>
            <template v-else>
              <button class="btn" type="button" @click="status = 'pending'">查看待处理</button>
            </template>
          </div>
        </div>
      </div>
...
const fetchList = async () => {
  ...
  loading.value = true
  try {
    const data = await adminApi.getReports({ pageNum: pageNum.value, pageSize: pageSize.value, status: status.value })
    const all = data.list || []
    list.value = all.filter((x) => x.targetType === 'comment')
    total.value = status.value === 'pending' ? list.value.length : data.total || 0
  } catch (e: unknown) {
...
const process = async (reportId: number, action: 'close' | 'deleteComment') => {
  const remark = window.prompt('处理备注（可选）') || undefined
  const ok = window.confirm(`确认执行动作：${action === 'close' ? '结案' : '删评并结案'} ?`)
  if (!ok) return
  actingId.value = reportId
  error.value = ''
  try {
