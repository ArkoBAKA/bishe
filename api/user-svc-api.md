...
- **Auth**: 不需要
- **角色**: `guest`
- **说明**: 账号密码注册，支持头像文件上传（后端上传 OSS 并落库 `avatarUrl`）
...
```json
{
  ...
  "data": {
    "userId": 10001,
    "avatarUrl": "https://oss.example.com/tieba/avatar/10001.png"
  }
}
```
...
- 有 `avatarFile` 时，后端先上传 OSS，得到 `avatarUrl` 后再写入用户表。

### 实现备注（给后端 Agent）
- 所属服务：gateway + user-svc
- 涉及实体：用户
- 事务策略：OSS 上传 + 单表写入事务（写库失败时可回收已上传对象，失败可仅记录日志）
- 幂等约束：账号唯一索引
...
- **Auth**: 需要
- **角色**: `user` / `admin`
- **说明**: 修改当前用户资料，支持头像文件上传（后端上传 OSS 并回写 `avatarUrl`）
...
| 40101 | 未登录或 Token 无效 | 鉴权失败 |
| 10031 | 参数非法 | 长度或格式不合法 |
| 10032 | 头像上传失败 | 上传 OSS 失败或文件格式非法 |

### 业务规则（实现必须遵守）
- 至少一个字段有变更才执行更新。
- 接口 `Content-Type` 使用 `multipart/form-data`。
- 有 `avatarFile` 时，后端上传 OSS 成功后回写 `avatarUrl`。

### 实现备注（给后端 Agent）
- 所属服务：gateway + user-svc
- 涉及实体：用户
- 事务策略：OSS 上传 + 单表更新事务（更新失败时可回收已上传对象，失败可仅记录日志）
- 幂等约束：重复提交同样内容不报错
