...
func DocsMarkdown() string {
	...
	b.WriteString("- 字段命名：`camelCase`\n")
	b.WriteString("- 时间格式：`RFC3339`\n")
	b.WriteString("- 分页参数：`pageNum`、`pageSize`（默认 `10`）\n")
	b.WriteString("- 列表返回：统一 `data.list + data.total`\n\n")
	b.WriteString("## 统一响应结构（需前后端共同遵守）\n\n")
	b.WriteString("所有接口响应体固定为：\n\n")
	b.WriteString("```json\n")
	b.WriteString("{\n")
	...
	b.WriteString("}\n")
	b.WriteString("```\n\n")
	b.WriteString("- `code=0` 表示成功；`code!=0` 表示失败\n")
	b.WriteString("- 失败时：HTTP 状态码为 4xx/5xx，`data` 固定返回空对象 `{}`\n")
	b.WriteString("- 列表成功返回建议：\n\n")
	b.WriteString("```json\n")
	b.WriteString("{\n")
	b.WriteString("  \"code\": 0,\n")
	b.WriteString("  \"message\": \"ok\",\n")
